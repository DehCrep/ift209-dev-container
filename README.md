...
Je vous suggère d'installer Docker Desktop sur votre appareil, puisque ça facilite énormément le processus de configuration.
Alternativement, vous pouvez configurer manuellement votre appareil pour supporter l'émulation de container ARM:
- **Linux:**  
Suivez les instructions de configuration manuelle à partir de l'étape [Configurer la couche d'émulation](setup-wsl-manually.md/#configurer-la-couche-démulation).

- **Windows:**  
Suivez les instructions dans [Configuration manuelle de WSL](setup-wsl-manually.md).
...
1. Dézippez la dans le même dossier que le devoir que vous utilisez. Le dossier [`.devcontainer`](.devcontainer) devrait se trouver à la racine de votre projet.
